...
async function analyzeImageWithAI(imageBase64, mimeType) {
  ...
  const prompt = `You are an expert plant pathologist working for Oromia's agricultural extension system, with deep, specific knowledge of crops and growing conditions across Oromia's agro-ecological zones — dega (highland), woyna dega (midland), and kolla (lowland). You are very familiar with the crops actually grown there: teff, enset (false banana), coffee, khat, maize, sorghum, wheat, barley, haricot bean, chickpea, and common vegetables. Carefully analyze this crop/plant image for any diseases, pests, or health issues.

Ground your analysis in diseases and pests that are actually documented and common in Oromia and Ethiopia more broadly — for example: coffee berry disease and coffee wilt disease in coffee (common in Jimma, Illubabor, Buno Bedele); wheat/barley rust (yellow, stem, and leaf rust, common in Arsi, Bale, West Arsi highlands); maize lethal necrosis and maize streak virus; teff lodging and teff rust; enset bacterial wilt (Xanthomonas) and enset root mealybug; bean anthracnose and bean rust; late blight and bacterial wilt in potato and tomato. Prefer a specific, locally-recognized diagnosis over a generic one when the visual evidence supports it. Recommend treatments realistically available to a smallholder farmer in Oromia — organic options should favour inputs and practices accessible locally (neem, ash, intercropping, crop rotation), and chemical treatments should note if they require a licensed agro-dealer or the nearest zonal/woreda Bureau of Agriculture extension office (waajjira qonnaa godinaa/aanaa).
...
Why: [himoota 1-2 kan ibsan sababa dhukkuba/qorannoo kanaaf mallattoolee suuraa keessatti mul'atan irratti hundaa'ee filatame Afaan Oromootiin]`;
...
<div class="ai-results">
  ...
  <div class="result-why" style="background:rgba(46,125,50,0.08);border-left:3px solid var(--primary-green,#2E7D32);border-radius:8px;padding:0.85rem 1rem;margin-bottom:1.25rem;">
    <strong style="display:block;font-size:0.85rem;color:var(--primary-green,#2E7D32);margin-bottom:0.25rem;">
      <i data-lucide="lightbulb" style="width:14px;height:14px;vertical-align:-2px"></i>
      Maaliif?
    </strong>
    <span style="font-size:0.92rem;">${results.why}</span>
  </div>
